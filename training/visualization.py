...
def visualize_on_input(model, input):
    W_E, W_pos, W_V, W_O, W_U = [arr.detach().cpu().numpy() for arr in (model.W_E, model.W_pos, model.W_V, model.W_O, model.W_U)]
    ...
    topk_vals, topk_idxs = topk.values.detach().cpu().numpy(), topk.indices.detach().cpu().numpy()
    ...
    attn = cache['attn', 0][0,0,-1].detach().cpu().numpy()
    ...
    resid_pre = cache['resid_pre', 0][0].detach().cpu() # (n_ctx, d_model)
    contribs = (resid_pre @ W_V @ W_O @ W_U)[0, 0].numpy() # (n_ctx, d_vocab)
...
def plot_weight_info(model):
    ...
    attn_weights = (model.W_E + model.W_pos[-1]) @ model.W_Q[0,0] @ model.W_K[0,0].T @ model.W_E.T
...
if __name__ == '__main__':
    ...
    visualize_on_input(model, torch.tensor([60]*4 + [40]))
# %%
