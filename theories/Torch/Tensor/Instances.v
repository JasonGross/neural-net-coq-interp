...
Module PArray.
  ...
  #[export] Instance reabstract_Proper {r A s} : Proper (eqf ==> eq ==> eqf) (@reabstract r A s).
  Proof. cbv [reabstract]; repeat intro; subst; destruct andb; eauto. Qed.

  #[export] Instance checkpoint_Proper {r A default s} : Proper (eqf ==> eqf) (@checkpoint r A default s).
  Proof. cbv [checkpoint]; repeat intro; subst; apply reabstract_Proper; try apply concretize_Proper; auto. Qed.
End PArray.
Export (hints) PArray.
...
Module List.
  ...
  #[export] Instance reabstract_Proper {r A default s} : Proper (eqf ==> eq ==> eqf) (@reabstract r A default s).
  Proof. cbv [reabstract]; repeat intro; subst; match goal with |- context[match ?x with _ => _ end] => destruct x end; eauto. Qed.
...
Proof.
  ...
  all: try eapply sum_Proper_pointwise.
...
Proof.
  ...
  1: eapply mean_Proper_pointwise.
