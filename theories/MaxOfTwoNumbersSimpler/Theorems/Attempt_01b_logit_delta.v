...
(*
  ...
  repeat (let v := fresh "v" in
          set (v := Reduction.argmax _ _ _ _) in *;
          let v' := fresh "v" in
          let H := fresh in
          remember v as v' eqn:H in *; subst v;
          symmetry in H;
          apply Reduction.argmax_spec in H).
  repeat match goal with
         | [ H := ?v |- _ ] => is_var v; subst H; rename v into H
         end.

  cbv [Reduction.in_bounds] in *.
  destruct_head'_and.

  move predicted_logits
  subst predicted_logits; cbv beta in *.
  move logits at bottom.
  ...
  set (k := to_Z _) in *; vm_compute in k; subst k.
  let v := (eval cbv delta [i'] in i') in
  change v with
  match goal with
  | [ H := context[v] |- _ ] => change v with i' in H
  end.
  subst true_maximum pred_tokens.

  vm_compute of_Z in *.
  RawIndex.curry_let_step ().
  specialize_step i'.
  ...
  move indices_of_max at bottom.
  move true_maximum after indices_of_max.
