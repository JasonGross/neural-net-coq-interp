From Coq Require Import Classes.Morphisms Relations.Relation_Definitions.
...
Ltac commute_option_rect_Let_In := (* pull let binders out side of option_rect pattern matching *)
  ...
  (* Kludge around COQBUG(https://github.com/coq/coq/issues/17846) *)
  Monomorphic Lemma option_leq_to_eq_to_leq {A x y} v : @option_eq_to_leq A x y (@option_leq_to_eq A x y v) = v.
  Proof.
    destruct x; subst; simpl; reflexivity.
  Qed.

  (* Kludge around COQBUG(https://github.com/coq/coq/issues/17846) *)
  Monomorphic Lemma option_eq_to_leq_to_eq {A x y} v : @option_leq_to_eq A x y (@option_eq_to_leq A x y v) = v.
  Proof.
    compute in *.
