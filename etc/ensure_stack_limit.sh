...
if command -v ulimit >/dev/null 2>/dev/null; then
    ...
    (
        ...
        { test "$recstacksize" != "unlimited" && test "$stacksize" -ge "$recstacksize"; } || { >&2 printf "Warning: Stack size (%s) may be too small.\n" "$stacksize"; return 1; }
    ) || {
        >&2 printf "Warning: To avoid stack overflows in OCaml native compilation, setting stack size to the recommended minimum of %s kbytes\n" "$recstacksize"
    ...
    >&2 printf "Warning: Could not determine stack size because ulimit was not found.\nWarning: We recommend a stack size of at least %s kbytes to avoid stack overflows in OCaml native compilation.\n" "$recstacksize"
fi
